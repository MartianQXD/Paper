...
\subparagraph*{Experimental Results and Improvement.}Table~\ref{tab:result} provides a summary of our experimental results over those four examples. In each case, we report the bisimilarity, the number of nodes in two pLTSs, the numbers of non-bisimilar and bisimilar state pairs in $N$ and $B$, as well as the verification time of our algorithm (excluding the pLTS generation part).
