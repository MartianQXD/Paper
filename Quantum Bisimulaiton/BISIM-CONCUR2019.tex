...
In order to show the correctness of the algorithm, we need to introduce some notation. 
%First, we let $(\theta, N)$ to be the return pair of functions, moreover $(\theta_{ij}, N_{ij})$ is the return of the $i\cdot j$-th execution of the function with state pair $(t_i,u_j)$. 
We use \texttt{R}$(t,u,W,N)$ to mean the following condition is satisfied:
\begin{itemize}
...
\hline
\rule{0pt}{12mm}&Teleportation & \tabincell{c}{$q1=|1\rangle$\\$q2=|0\rangle$\\$q3=|0\rangle$} & Yes & 33 & 15 & 0 & 2.7 \\
\hline
