...
The tool is available from \url{https://github.com/MartianQXD/QBisim}, where we also provide all the examples for the experiments to be discussed in Section~\ref{sec:exper}.
...
%BB84 is the first quantum key distribution protocol developed by Bennett and Brassard in 1984~\cite{BB84}. It 
...
For simplicity, we assume the sequence $\tilde{q}$ consists of only one qubit. This is enough to reflect the essence of the protocol. The other qubits used below are auxiliary qubits for the operation  $Ran$.
\begin{flalign*}
...
\subparagraph*{Inputs.}
For the implementation $BB84$, we need to declare  the following variables and operators in the inputs attached to it.
\begin{itemize}
