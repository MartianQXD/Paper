...
\keywords{Quantum processes, Bisimulation, Algorithm.}%TODO mandatory; please add comma-separated list of keywords
...
One important application of quantum process algebras is to formally verify quantum communication protocols. With a suitable notion of behavioural equivalence and a decision method, one can determine if the specification of a protocol is consistent with an implementation. Ground bisimulation is a convenient behavioural equivalence for processes in quantum CCS because of its associated coinduction proof technique. We exploit this technique to design and implement an on-the-fly algorithm to check if two given quantum processes are equivalent. This enables us to develop a tool that can verify interesting quantum protocols such as the BB84 quantum key distribution scheme.
\end{abstract}
...
An auxiliary function \textbf{Act}$(t,u)$ is called in \textbf{Match} to discover the next action that both $t$ and $u$ can perform. If both states have no more action to do the function returns an empty set. If only one of them has no more action to do then the two states are immediately declared to be non-bisimilar. 
...
The other set $W$ is updated in function \textbf{MatchAction}$(\gamma,t,u,W)$. This function discovers next state pairs according to the action $\gamma$ and recursively invokes the function \textbf{Match} when there is a pair of states or \textbf{MatchDistribution} when there is a pair of state distributions. The current state pair is added to $W$ when the new function is invoked.
...
\begin{tabular}{@{}m{0pt}@{}
                ...
                |>{\centering\arraybackslash}m{1cm}}
\hline
\rule{0pt}{5mm}&\textbf{Program} & \textbf{Variables} & \textbf{Bisim} & \textbf{MN} & \textbf{SN} & \textbf{Size of $N$} & \textbf{Sec} \\
\hline
\hline
...
In this paper, we have presented an on-the-fly algorithm verifying strong ground bisimulation for quantum programs in qCCS. Furthermore, we have developed a tool for bisimilarity checking basing on the algorithm. To show its performance, we further made experiments on several quantum communication protocols such as BB84. 
% The result shows that the algorithm can figure out the non-bisimilarity and help us make improvement on the program. As an illustration, in verifying super-dense coding protocol, we found that the program we coded missed the case that an unexpected input value is given. Furthermore, we ensured that the program after modifying fulfills all the cases.
The experimental results have showed the algorithm is able to give hints for two kinds of improvement: (1) supplementing the missing cases of the programs; (2) adjusting the parallel behaviours.
